...
let gGMC_DEBUG=false;let gMarkCodeLineManager=new MarkCodeLineManager();window.addEventListener("DOMContentLoaded",_=>{gMarkCodeLineManager.setElement();gmcExpandNavigation();gmcExternalLinks();});window.addEventListener("hashchange",_=>{gMarkCodeLineManager.setElement();})
...
const activeNav=document.querySelector(".md-nav__link--active").parentElement;const children=activeNav.querySelector("nav > ul").children;for(let i=0;i<children.length;i++){const toggle=children[i].querySelector('input[type="checkbox"]');if(!toggle){continue;}
